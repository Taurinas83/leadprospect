...
# Build if needed (production mode for stability)
if [ ! -d ".next" ] || [ ! -f ".next/BUILD_ID" ]; then
  log_step_start "Building Next.js"
  echo "[BUILD] Running next build..."
  npx next build
  log_step_end "Building Next.js"
fi

log_step_start "Starting Next.js server"
echo "[NODE] Starting production server..."
node server.js &
DEV_PID=$!
...
echo "[NODE] Performing health check..."
...
echo "Next.js server is running in background (PID: $DEV_PID)."
